...
export async function DELETE( request: NextRequest, { params }: { params: { id: string } })
{
	try
	{
		...
		const existingProduct = await prisma.task.findUnique(
		{
			where: { id }
		})
	...
	try
	{
		...
		if (!title || typeof title !== 'string' || title.trim().length === 0) {
			return NextResponse.json(
				{
					success: false,
					error: 'Le titre du produit est requis et doit être une chaîne non vide'
				},
				{
					status: 400
				}
		...
		if (!dueDate || typeof dueDate !== 'string')
		{
			return NextResponse.json(
				{
					success: false,
					error: 'La date d\'échéance doit être une chaîne de caractères'
				},
				{
					status: 400
				}
		...
		const updatedProduct = await prisma.task.update(
		{
			where: { id },
			data:
			{
